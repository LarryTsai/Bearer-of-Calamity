...
- 012傷休新增002～004、013三城四章已完成；R0陸返閣後R2～R10復養、R11同場聯市、R12實爐配片，接R13偏城。右臂仍固定，宋／阿自己的藥引留閣，未額外加半年。顧問讀回未確認新增P0/P1。見 [生活第三批](drafts/LOWER_LIFE_PHASE3_COMPLETION.md)；下一批014／015，之後R18拆板、R21承原016。
...
- 現行規模：43章群、900來源章／900閱讀章、997生成檔；下界24篇／89章，空篇明列待開發。每階段驗證後commit並push；風格精修後置。
...
### 現行目錄與本輪提交
- 正文入口以 `docs/STORY_STRUCTURE.json` 為準：43個篇章群、900章，各層三位數排序；可讀對照見 `docs/STORY_STRUCTURE.md`。本板以下保留的 volume／arc 編號是歷史定位，派工須先用 manifest 的 `legacy_source` 對到現行 `source`／`published`，不能直接沿舊路徑施工。
- PM已推送：`25cd0cb`共同規則、`f6dda22`鏡像10檔修復、`bbe2e28`百斷山8章審查、`9d6dcfc`古界083／085／086局部修訂。目錄重整與工具變更已提交 `abdfe94`；遠端同步狀態以Git收尾查核為準。
- 現行驗證命令為 `python scripts/build_story.py --check`，核對900章正文、閱讀版、997個生成檔與43篇大綱；舊稿已清除，不使用舊檔例外。
