...
- 本批完成古界087〈先走的人〉、088〈自己過去〉、089〈不在原處的地方〉，N-2正式出界與暫返抉擇已落文，顧問必要硬錯覆核無新增P0/P1。實際抵達三千州、N-3殘鐧共鍛及無兵開篇接縫是下一施工段。
- [下界24篇草案](outlines/LOWER_REALM_LIFE_MASTER_OUTLINE.md)維持設計依據，洛生衣保留上界初遇；不再以完善全部場景大綱為寫正文的前置條件。

### 現行目錄與本輪提交
- 正文入口以 `docs/STORY_STRUCTURE.json` 為準：28個篇章群、870章，各層三位數排序；可讀對照見 `docs/STORY_STRUCTURE.md`。本板以下保留的 volume／arc 編號是歷史定位，派工須先用 manifest 的 `legacy_source` 對到現行 `source`／`published`，不能直接沿舊路徑施工。
- PM已推送：`25cd0cb`共同規則、`f6dda22`鏡像10檔修復、`bbe2e28`百斷山8章審查、`9d6dcfc`古界083／085／086局部修訂。目錄重整與工具變更已提交 `abdfe94`；遠端同步狀態以Git收尾查核為準。
- 現行驗證命令為 `python scripts/build_story.py --check`，核對870章正文、閱讀版、937個生成檔與28篇大綱；舊稿已清除，不使用舊檔例外。
...
## 一、現況摘要
- 主線大部已寫至第十九篇＋戰後沉封域；**古界N-2已完成，返程／N-3仍未完成**。古界入口 `novel/002_上界成道/009_界潮與古界/README.md`；ch080～089已寫，N2-7拆為087～088、N2-8為089；N-3未寫，不得標成全書完稿。
