...
- 014偏城一個完整長章、015爭爐三章已落文：R13唐岫驗埋城，R14～R17孟禾爭得下一期北間爐位，R17返閣、R18拆板，銜原016拆板三日。見[生活第四批](drafts/LOWER_LIFE_PHASE4_COMPLETION.md)。017海市三章、018四章修士能力修正已完成；下一批019～021返閣／遊藝／昇龍台，見[具體施工路線](drafts/LOWER_SEA_AND_RETURN_NEXT_PASS.md)。
...
- 現行規模：43章群、907來源章／907閱讀章、1004生成檔；下界24篇／96章，空篇明列待開發。每階段驗證後commit並push；風格精修後置。
...
### 現行目錄與本輪提交
- 正文入口以 `docs/STORY_STRUCTURE.json` 為準：43個篇章群、907章，各層三位數排序；可讀對照見 `docs/STORY_STRUCTURE.md`。本板以下保留的 volume／arc 編號是歷史定位，派工須先用 manifest 的 `legacy_source` 對到現行 `source`／`published`，不能直接沿舊路徑施工。
- PM已推送：`25cd0cb`共同規則、`f6dda22`鏡像10檔修復、`bbe2e28`百斷山8章審查、`9d6dcfc`古界083／085／086局部修訂。目錄重整與工具變更已提交 `abdfe94`；遠端同步狀態以Git收尾查核為準。
- 現行驗證命令為 `python scripts/build_story.py --check`，核對907章正文、閱讀版、1004個生成檔與43篇大綱；舊稿已清除，不使用舊檔例外。
