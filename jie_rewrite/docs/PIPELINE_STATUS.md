...
- 戰後〈古龍界・自己的巢〉五章已落文：接劫後032，厄自行赴回潮巢群、付料共造朝雲巢、住兩夜後回家，約半月另計；沅枝與棠礁各自選巢位。第226題獨立龍族故事本輪已有具體入口、地方歷史、自主人物、厄的選擇及花用，非飛升資格；下一階段正式抵仙界。見[篇綱](outlines/OUTLINE_DRAGON_HOME.md)。

- 上界第二昇龍台落於長息020～026：劫瘋各自追問折嶺海、分路登頂，石生修士岑石取回山音；正常故事完成後才取得灰白弧片，與下界黑褐窄片首次共鳴。第一件仍由陸沉保管、第二件由封寒保管；不揭用途、不開門。原020順移027並依既定裁示重寫正式黑洞成道，固定鐧、不提前白洞。見[本批驗收](drafts/UPPER_TERRACE_COMPLETION.md)。
...
- 014偏城一個完整長章、015爭爐三章已落文：R13唐岫驗埋城，R14～R17孟禾爭得下一期北間爐位，R17返閣、R18拆板，銜原016拆板三日。見[生活第四批](drafts/LOWER_LIFE_PHASE4_COMPLETION.md)。017海市三章、018四章修士能力修正已完成；019返閣四章與日常洞天突破已完成；020遊藝兩章也已完成；021昇龍台六章與古物實體保管接縫已完成；接續上界第二台、必要長線與正式抵仙界，見[具體施工路線](drafts/LOWER_SEA_AND_RETURN_NEXT_PASS.md)。
...
- 現行規模：44章群、931來源章／931閱讀章、1030生成檔；下界24篇／108章，各篇均有正文，長息47章、古龍界5章。每階段驗證後commit並push；下界、上界兩台及厄的自主龍族短篇已落文，正式抵仙界仍未完，風格精修後置。
...
### 現行目錄與本輪提交
- 正文入口以 `docs/STORY_STRUCTURE.json` 為準：43個篇章群、926章，各層三位數排序；可讀對照見 `docs/STORY_STRUCTURE.md`。本板以下保留的 volume／arc 編號是歷史定位，派工須先用 manifest 的 `legacy_source` 對到現行 `source`／`published`，不能直接沿舊路徑施工。
- PM已推送：`25cd0cb`共同規則、`f6dda22`鏡像10檔修復、`bbe2e28`百斷山8章審查、`9d6dcfc`古界083／085／086局部修訂。目錄重整與工具變更已提交 `abdfe94`；遠端同步狀態以Git收尾查核為準。
- 現行驗證命令為 `python scripts/build_story.py --check`，核對926章正文、閱讀版、1023個生成檔與43篇大綱；舊稿已清除，不使用舊檔例外。
...
3. 長息 G 與各卷質感精修移到正文補齊後；原020／現027黑洞成道已依既有裁定重寫，落實有限維持、解除、重現及永久手部精細感知代價；其餘 G 項另行精修。
