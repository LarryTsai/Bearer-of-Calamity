...
如果不確定自己在這個專案裡的角色，**先讀 `docs/PIPELINE_STATUS.md`**——那裡記錄了目前是誰在當PM、誰是寫手、每個人手上的任務與今天的關鍵決策。

如果使用者告訴你「你是寫手1」或「你是寫手2」，接著讀對應的 `docs/PIPELINE_WRITER1_LOG.md` 或 `docs/PIPELINE_WRITER2_LOG.md`——裡面有你的角色定義、標準作業流程、必須遵守的長期寫作原則，以及你自己過去完成過什麼、卡在哪。讀完就能直接接續工作，不需要使用者重新解釋。

如果使用者告訴你「你是PM」，除了讀 `PIPELINE_STATUS.md`，也要負責在每次重大進度後更新它，讓其他人（含未來/clear後的你自己）能對上現況。

**這個機制的前提是使用者會在/clear後告訴你「你是誰」**——本檔案本身會自動載入，但對話記錄與session顯示名稱是否在/clear後保留，取決於實際的Claude Code環境行為，不能假設一定會自動延續；如果使用者只說「繼續」而沒有講你是誰，先問清楚，不要用猜的。
